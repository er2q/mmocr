...
request_time = {}       # 记录请求次数
now_time = time.strftime("%Y-%m-%d", time.localtime(time.time()))       # 服务开启的时间
...
class TrRun(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        try:
            if hasattr(img, '_getexif') and img._getexif() is not None:     # 旋转
                orientation = 274
        ...
        if remote_ip_now not in request_time:       # 同一IP一天访问次数限制
        ...
        if max(img_w, img_h) * (short_size * 1.0 / min(img_w, img_h)) > dbnet_max_size:     # TODO
            # logger.error(exc_info=True)
            res.append("图片resize后长边过长，请调整短边尺寸")
            do_det = False
            # self.finish(json.dumps({'code': 400, 'msg': '图片resize后长边过长，请调整短边尺寸'}, cls=NpEncoder))
            # return
